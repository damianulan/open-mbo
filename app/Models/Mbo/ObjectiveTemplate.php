 ...
 * @method static \YMigVal\LaravelModelCache\CacheableBuilder<static>|ObjectiveTemplate whereUuid($value)
...
class ObjectiveTemplate extends BaseModel implements HasObjectives
{
    ...
    public function usersCount(): int
    {
        if ($this->relationLoaded('objectives')) {
            return $this->objectives->sum(function (Objective $objective): int {
                if (isset($objective->user_objectives_count)) {
                    return (int) $objective->user_objectives_count;
                }

                if ($objective->relationLoaded('user_objectives')) {
                    return $objective->user_objectives->count();
                }
        ...
        return UserObjective::query()
            ->whereIn('objective_id', $this->objectives()->select('id'))
            ->count();
    }
