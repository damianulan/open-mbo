<?php

namespace Database\Seeders;

use App\Models\Business\Company;
use App\Models\Business\Department;
use App\Models\Business\Position;
use App\Models\Business\TypeOfContract;
use App\Models\Business\UserEmployment;
use App\Models\Core\User;
use App\Models\Core\UserProfile;
use Illuminate\Database\Eloquent\Builder;
use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;
use Throwable;

class UserSeeder extends Seeder
{
    public function run(): void
    {
        if (UserEmployment::query()->exists()) {
            return;
        }

        $companies = Company::query()->with('departments')->get();
        $contracts = TypeOfContract::query()->get();
        $managementPositions = Position::query()->whereIn('name', ['CEO', 'CTO', 'CFO'])->get()->keyBy('name');
        $employeePositions = Position::query()->whereNotIn('name', ['CEO', 'CTO', 'CFO'])->get();

        if ($companies->isEmpty() || $contracts->isEmpty() || $employeePositions->isEmpty()) {
            return;
        }

        try {
            DB::beginTransaction();
            foreach ($companies as $company) {
                if ($company->departments->isEmpty()) {
                    continue;
                }

                $num = fake()->numberBetween(60, 80);
                $admins_mbo = 5;
                $superadmins = 3;

                for ($i = 1; $i <= $num; $i++) {
                    $user = User::factory()
                        ->has(UserProfile::factory()->count(1), 'profile')
                        ->create();

                    $position = null;
                    if ($user) {
                        $user->assignRoleSlug('employee');
                    }

                    if ($admins_mbo > 0) {
                        $user->assignRoleSlug('admin_mbo');
                        $admins_mbo--;
                    } elseif ($superadmins > 0) {
                        $user->assignRoleSlug('admin');
                        $positionKey = match ($superadmins) {
                            2 => 'CTO',
                            3 => 'CFO',
                            default => 'CEO',
                        };
                        $position = $managementPositions->get($positionKey);
                        $superadmins--;
                    }

                    $isManagement = true;
                    if ( ! $position) {
                        $position = $employeePositions->random();
                        $isManagement = false;
                    }

                    $chance = fake()->numberBetween(1, 3);
                    UserEmployment::create([
                        'user_id' => $user->id,
                        'company_id' => $company->id,
                        'contract_id' => $contracts->random()->id,
                        'position_id' => $position->id,
                        'department_id' => $company->departments->random()->id,
                        'employment' => fake()->dateTimeBetween('-10 years', '-3 months'),
                        'release' => 3 === $chance && ! $isManagement ? fake()->dateTimeBetween('-10 months', '+1 year') : null,
                    ]);
                }

                $company->departments->each(function (Department $department) use ($company): void {
                    User::whereHas('employments', function (Builder $query) use ($company): void {
                        $query->whereNull('release');
                        $query->where('company_id', $company->id);
                    })->take(1)->get()->each(function (User $supervisor) use ($company, $department): void {
                        $supervisor->assignRoleSlug('supervisor');

                        User::whereHas('employments', function (Builder $query) use ($company, $department): void {
                            $query->whereNull('release');
                            $query->where('company_id', $company->id);
                            $query->where('department_id', $department->id);
                        })->get()->each(function (User $user) use ($supervisor): void {
                            $user->refreshSupervisors([$supervisor->id]);
                        });
                    });
                });
            }

            DB::commit();
        } catch (Throwable $th) {
            DB::rollBack();
            throw $th;
        }
    }
}
